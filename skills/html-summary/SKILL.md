...
hairline, no radius, no shadow, dense — that does not bend. Which sections exist,
in what order, built from which components: you decide, from what the content
actually is. If you are filling a component instead of choosing one, delete the
component.
...
Peer cells sit either **together** or **apart**, and that is a decision about
spacing, not about what goes in them. A cost strip and a habits grid can each be
either form.

**Stack (joined)** = one compound unit. Cells share edges (1px gap over `--line`,
or collapsed borders), no gutter between peers. Use it for dense peer data where
the comparison is the point — a row of metrics, a ranking.

**Card (separated)** = peers with a gap between them, each panel standing alone
(usually on `--bg-sub`). Use it when each peer needs air to be read on its own.

Pick one per group and hold it. Mixing both inside a single group reads as an
accident.
...
This is the standalone design system and it depends on nothing.
[Lattice](https://github.com/yeksax/lattice) — the local app that serves
`~/.summaries` — adds theme configuration, a searchable dashboard, live reload
while you write, one-command public sharing, and interactive polls/bake-offs
that collect votes when the summary is served through it. The lattice-integrated
version of this skill ships with that CLI (`lattice skills install`) and
documents those hooks.
